...
if (isset($_POST['do-send'])) {
    if (empty($_POST['mokhatab'] && $_POST['matn'])) {
        ...
        if ($register) {
            $check = mysqli_query($db, "SELECT * FROM db_chat WHERE fname='$mokhatab' OR gname='$mokhatab'");
            if (mysqli_num_rows($check) > 0) {
                $_SESSION['payam'] = $mokhatab;
                header('Location: payam_shakhsi.php');
...
?>
    <form action="engine/search.php" method="get" class="warp">
        ...
        <div class="col-xs-12 col-sm-12">
            <div class="panel panel-warning">
                <div class="panel-heading">مشاهده ی پیام ها</div>
                <div class="panel-body">
                    <form action="engine/mosh_shakhsi.php" method="post">
                    ...
                    <script>
                        $(document).ready(function (e) {
                            function showmsg() {
                                $.post("engine/mosh_shakhsi.php",function (data) {
                                    $("#result").html(data);
                                });
                            }
